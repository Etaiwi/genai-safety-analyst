...
def get_history_html() -> str:
    ...
    for item in request_history:
        ...
        color = {"ALLOWED": "#10b981", "FLAG": "#f59e0b", "BLOCK": "#ef4444"}.get(label, "#6b7280")
        ...
        " onclick="loadFromHistory('{item['id']}')" onmouseover="this.style.background='#f1f5f9'" onmouseout="this.style.background='#f8fafc'">
            ...
            {f'<div style="font-size: 10px; color: #6b7280; margin-bottom: 2px;">{item["category"].title()}</div>' if item.get("category") else ''}
            <div style="
                font-size: 12px;
                color: #374151;
                margin-bottom: 2px;
